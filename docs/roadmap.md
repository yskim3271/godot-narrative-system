...
2. ~~하단 패널 고도화~~ ✅ — ~~대화 미리보기(에디터 내 재생, 샌드박스 컨텍스트)~~ · ~~누락 번역 일괄 표시(Localization 탭, 로케일 필터)~~ · ~~검증/번역 이슈 더블클릭 → Inspector + 그래프 노드 포커스~~
...
2. ~~퀘스트: abandon/반복 퀘스트(완료 횟수 추적), objective 자동 완료 조건, 카테고리~~ ✅ (저장 스키마 v2 + 1→2 마이그레이션)
